...
inline void mywrite(unsigned long n)
{
	asm volatile("mov %0,%%cr0": "+r" (n),"+m"(__force_order));
}
...
static int __init hello_init(void)
{
	...
	kstrtoul(input,16,&p);
        p1= p;
	fork_addr=p1[__NR_clone];


	printk(KERN_ALERT "%lu\n", fork_addr);
	fork_fun=p1[__NR_clone];

	write_cr0(read_cr0() & (~0x10000));

	p1[__NR_clone]=(sys_call_ptr_t*)hook_fun; 
...
static void __exit hello_cleanup(void)
{
	...
	p1[__NR_clone]=(sys_call_ptr_t*)fork_addr;
